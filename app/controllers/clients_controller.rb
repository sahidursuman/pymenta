class ClientsController < ApplicationController
  ...
  def search
   @clients = Client.where("domain = ? AND code like ? AND name like ? AND city like ? ", current_user.domain,"%#{params[:code]}%","%#{params[:name]}%","%#{@city}%").paginate(:page => params[:page], :per_page => 10).order('name ASC')
   render :index
  end
